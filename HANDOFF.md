...
- The watchlist renders folder headers with counts and tag chips, and a `⌗` control on each row files a symbol via prompts (matching the existing `setAlert` convention). It stays **flat with no headers until something is actually filed**, so an unfiled watchlist is visually unchanged.
- The **fundamental analyst** reads promoter-stake trend from `shareholding` via `shareholding_trend()`. Rising = insiders buying, falling = the warning; stance is `change/5.0` capped at ±0.6 because a stake also moves through dilution, pledging or an OFS, none of which is a forecast. **It abstains on every symbol until the ingester is deployed and has run twice** — one quarter gives a level, two give the trend.
- **User investment profile**: `risk_tolerance` (conservative/balanced/aggressive) and `investment_style` (value/growth/momentum/income/balanced) on `users`, served by `GET|POST /api/me/preferences`. Vocabularies are closed. The write path **rejects** an unknown value (so a typo is reported) while the read path **coerces** (so no caller ever gets an unusable string) — that asymmetry is deliberate. **Nothing consumes them yet**: `v2_web` has no user context, so `api_stock` cannot tell who is asking.
- **Shareholding is ingestible; the rest of fundamentals is not.** `scripts/shareholding_ingest.py` pulls promoter/public/employee-trust percentages for ~2,275 companies in ONE NSE request and stores quarterly history in `shareholding` (deliberately **no pruning** — the trend is the signal). Revenue/EPS/PE/ROE are **blocked**: NSE's results API carries metadata only, with figures behind ~3,800 per-record links, and Yahoo's `quoteSummary` returns 401 without a crumb. Note promoter+public need not sum to 100 — employee trusts and DRs hold the remainder.
- **Watchlist rows carry a `folder` and `tags`** (`v2_watch_user`, added by guarded ALTER). Both are lower-cased and de-duplicated on write so "Banks" and "banks" are one group. Re-adding a symbol re-files it. `GET /v2/api/watchlist` returns them plus a `folders` count map. **Note:** inserts into that table must name their columns — the original positional `VALUES(?,?,?)` breaks against the wider table, and a test asserts it now raises.
- **Alerts fire server-side.** `_check_alerts()` used to be called ONLY from `_stream_payload()`, the SSE builder, so an alert was evaluated only while a browser had the dashboard open — set one, close the tab, and it never triggered. The engine loop now evaluates them every `ALERT_INTERVAL` (20s) via a **lazy** `v2_web` import (module-level would be circular, since `v2_web` imports `v2_live`). Kinds: above / below / pct-move, **cross_up**/**cross_down** (price crossing the 20/50/200-day SMA — a true cross, needing the previous close on one side and the live price on the other, so it fires once rather than every cycle while price stays across), **pattern** (optionally filtered to named patterns via a `patterns` list on create — unfiltered fires on 32% of names in a session, reversal-only on 13%; the vocabulary is `indicators.CANDLESTICK_PATTERNS` and an unknown name is rejected, not dropped; on a bar *newer than the alert* — daily patterns persist all session, so without the freshness rule it would re-fire every 20s until midnight), plus **catalyst** — which ignores price and fires on the first material NSE filing (`results`/`order`/`corp_action`) published *after* the alert was created, using the same materiality vocabulary as the engine's own gate. Delivered by Telegram. `alert_hit()` fails **closed** on bad input — a spurious fire is a false signal to the user's phone.
...
- The stock page renders the analyst panel: consensus, a centre-zero stance bar per analyst, rationales, collapsible evidence, and a highlighted **"analysts disagree"** block when the CIO reports dissent. Abstainers render as *abstained*, never as a neutral vote.
- **Analyst panel** (`app/analysts.py`, served as `recommendation.panel`): six independent analysts — technical, catalyst, fundamental, risk, macro, position — each with its own stance, confidence and evidence, reconciled by a CIO. Two deliberate properties: an analyst with no data **abstains** and is excluded rather than counted as a neutral vote, and the risk **and macro** analysts' stances are capped at zero so it can veto enthusiasm but never manufacture it. The CIO reports **dissent** explicitly and lowers its confidence when analysts genuinely conflict — the thing a single blended score cannot express. Display-only.
