...
- The local `.venv` has no `numpy`, so the v2 modules (`v2_live`, `v2_web` — the live engine and web app) silently fail to import and are **not covered** by the local test run. `unittest discover` reporting OK does not mean the trading engine was tested.
