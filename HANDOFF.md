...
8. **🔴 Daily candle ingestion is a session behind (found 2026-07-27 22:5x IST).** `max(ts)` across the whole `upstox-live:day` source was **2026-07-24**, hours after Monday's close — Monday's bar was never ingested. KFINTECH's last candle read ₹857.75 while the live quote was ₹950.40, a **10.8%** divergence, because the catalyst move happened on the missing session. This is not cosmetic: `_panel()`/`eng.compute_features()` build the engine's signals from these candles, so entry decisions can be made on a stale picture. Check `opentrade-candles.service` / `.timer` and `scripts/candle_ingest.py`. The stock page now labels its technicals with an `as_of` date and a `stale` flag, but that only *reports* the problem — it does not fix ingestion.
