...
function renderDesk(d){
 ...
 var explanation=risk.halted?risk.reason:(regime==='OFF'?'The market regime blocks new equity longs.':(regime==='UNKNOWN'?'Waiting for a current, completed-session market reading.':'Only candidates that pass data, conviction and account risk checks can enter.'));
 h+='<div class=desk-grid><section class=desk-panel><div class=desk-head><div class=desk-eyebrow>STRATEGY STATUS</div><span class="desk-status '+(blocked?'warn':'')+'">'+(blocked?'NO NEW ENTRIES':'SCREENING')+'</span></div><h2 style="font-size:23px;margin:8px 0">'+regime+' regime</h2><p class=desk-message>'+esc(explanation)+'</p><p class=desk-note>Existing positions retain exit management. A trading halt does not erase previous losses.</p><button class=desk-action style="margin-top:15px" onclick="go(\'ideas\')">Review opportunities →</button></section>'
