...
class Settings:
    ...
    us_full_decision_target: int = _int("US_FULL_DECISION_TARGET", 200)
    us_scanner_slot_budgets: str = os.getenv(
        "US_SCANNER_SLOT_BUDGETS",
        "live_rally=45,volume_price=40,breakout=40,earnings_news=30,sector_rs=25,diverse=20",
    ).strip()
    dynamic_scan_min_score: float = _float("DYNAMIC_SCAN_MIN_SCORE", 0.0)
    dynamic_scan_require_active_setup: bool = _bool("DYNAMIC_SCAN_REQUIRE_ACTIVE_SETUP", False)
    decision_authority_mode: str = "raw_opportunity_v1"
    ...
    missed_move_min_move_pct_us: float = _float("MISSED_MOVE_MIN_MOVE_PCT_US", 3.0)
    missed_move_review_market: str = os.getenv("MISSED_MOVE_REVIEW_MARKET", "BOTH").strip().upper()
    ...
    whatsapp_api_base_url: str = os.getenv("WHATSAPP_API_BASE_URL", "https://graph.facebook.com/v25.0").rstrip("/")
    ...
    whatsapp_test_template_name: str = os.getenv("WHATSAPP_TEST_TEMPLATE_NAME", "hello_world").strip()
    whatsapp_template_language_code: str = os.getenv("WHATSAPP_TEMPLATE_LANGUAGE_CODE", "en_US").strip()
    whatsapp_alert_template_name: str = os.getenv("WHATSAPP_ALERT_TEMPLATE_NAME", "").strip()
    whatsapp_alert_template_language_code: str = os.getenv("WHATSAPP_ALERT_TEMPLATE_LANGUAGE_CODE", "en_US").strip()

    initial_cash_inr: float = _float("INITIAL_CASH_INR", 10_000)
    max_positions: int = _int("MAX_POSITIONS", 3)
    max_position_pct: float = _float("MAX_POSITION_PCT", 0.34)
    ...
    daily_loss_limit_pct: float = _float("DAILY_LOSS_LIMIT_PCT", 0.015)
    ...
    paper_risk_per_trade_pct: float = _float("PAPER_RISK_PER_TRADE_PCT", 0.005)
    paper_min_auto_follow_notional_inr: float = _float("PAPER_MIN_AUTO_FOLLOW_NOTIONAL_INR", 1_500.0)
...
CONFIG_SCHEMA: list[dict[str, Any]] = [
    ...
    {"key": "whatsapp_test_template_name", "label": "WhatsApp Test Template", "type": "text", "category": "Notifications"},
    {"key": "whatsapp_template_language_code", "label": "WhatsApp Test Language", "type": "text", "category": "Notifications"},
    {"key": "whatsapp_alert_template_name", "label": "WhatsApp Alert Template", "type": "text", "category": "Notifications"},
    {"key": "whatsapp_alert_template_language_code", "label": "WhatsApp Alert Language", "type": "text", "category": "Notifications"},
    ...
    {"key": "us_full_decision_target", "label": "US Full Decisions", "type": "number", "category": "Market Data", "min": 1, "step": 10},
    ...
    {"key": "decision_authority_mode", "label": "Decision Authority", "type": "select", "category": "Market Data", "choices": ["raw_opportunity_v1"]},
    ...
    {"key": "missed_move_min_move_pct_us", "label": "US Missed Move %", "type": "number", "category": "Market Data", "min": 0, "step": 0.25},
...
def coerce_setting_value(key: str, value: Any, base: Settings) -> Any:
    ...
    if key == "whatsapp_api_base_url":
        return str(value).strip().rstrip("/") or "https://graph.facebook.com/v25.0"
    ...
    if key in {
        ...
        "whatsapp_alert_template_language_code",
    }:
