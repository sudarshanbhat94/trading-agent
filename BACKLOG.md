...
- [ ] `BLOCKED (no cheap source)` **Revenue / EPS / PE / ROE / debt.**
      Investigated 2026-07-28. NSE's quarterly-results API returns only
      metadata — `params` is a concatenated string, and the figures sit behind
      a per-record `resultDetailedDataLink` or XBRL, i.e. ~3,800 fetches per
      quarter. Yahoo's `quoteSummary`, the usual free substitute, now returns
